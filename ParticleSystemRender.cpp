...
#include "C:/Users/meteor/Desktop/FleX/FleX/Engine/Intermediate/ProjectFiles/ParticleWorldInformation.h"
...
{
	float totalX = 0, totalY = 0, totalZ = 0, avgSimPos_X = 0, avgSimPos_Y = 0, avgSimPos_Z = 0; //METEOR MOD CODE
	...
	for (int32 i = 0; i < ParticleCount; i++)
	{
		...
		//METEOR MOD CODE (begin)---------------
		totalX += ParticlePosition.X;
		totalY += ParticlePosition.Y;
		totalZ += ParticlePosition.Z;
	}
	meteor_posX = totalX / ParticleCount;
	meteor_posY = totalY / ParticleCount;
	meteor_posZ = totalZ / ParticleCount;
	TArray<IMotionController*> MotionControllers = IModularFeatures::Get().GetModularFeatureImplementations<IMotionController>(IMotionController::GetModularFeatureName());
	/*if (MotionControllers.Num() > 0) {
		IMotionController* motcon = MotionControllers[0];
		FName motconname = (&motcon).GetMotionControllerDeviceTypeName;
		//UE_LOG(LogTemp, Warning, TEXT("%s, X:%f\tY:%f\tZ:%f\t"), motcon->GetMotionControllerDeviceTypeName(),avgSimPos_X, avgSimPos_Y, avgSimPos_Z);
		UE_LOG(LogTemp, Warning, LINE_TERMINATOR);
	}*/
	//GetPawn();
	for (auto MotionController : MotionControllers)
	{
		...
		{
			FName fn = MotionController->GetMotionControllerDeviceTypeName();
			FString str = fn.ToString();
			FRotator MotConOrientation;
			FVector MotConPosition;
			float WorldToMetersScale = 100.0f;
			MotionController->GetControllerOrientationAndPosition(0, EControllerHand::Right, MotConOrientation, MotConPosition,100.0f);

			if (counter_meteor % 60 == 0) {

				//UMotionControllerComponent* LeftMotionController = CreateDefaultSubobject<UMotionControllerComponent>(TEXT("LeftMotionController"));
				//UWorld* wp = GEngine->GetWorld();
				//ParticleWorldInformation::posX = avgSimPos_X;
				//if (wp != NULL) {
				//}
				UE_LOG(LogTemp, Warning, TEXT("Particle X:%f\tY:%f\tZ:%f\t"), meteor_posX, meteor_posY, meteor_posZ);
				UE_LOG(LogTemp, Warning, TEXT("MotCon X:%f\tY:%f\tZ:%f\t"), MotConPosition.X, MotConPosition.Y, MotConPosition.Z);
				UE_LOG(LogTemp, Warning, TEXT("MotConRot R:%f\tP:%f\tY:%f\t"), MotConOrientation.Roll, MotConOrientation.Pitch, MotConOrientation.Yaw);
				//UE_LOG(LogTemp, Warning, TEXT("Diff X:%f\tY:%f\tZ:%f\t"), MotConPosition.X - avgSimPos_X, MotConPosition.Y - avgSimPos_Y, MotConPosition.Z - avgSimPos_Z);
				//MotionController->GetControllerOrientationAndPosition(0, EControllerHand::, MotConOrientation, MotConPosition, 1.0f);

				//PRINT TO FILE: Comma-delimited, with semi-colons to separate out rows. We will work on the coordinate transform in MATLAB.
				//timestamp
				//avgSimPos_X, Y, Z
				//MotConPos.X,Y,Z
				//MotConRotation.Yaw,Pitch,Roll
				
				//UE_LOG(LogTemp, Warning, LINE_TERMINATOR);
			}
			counter_meteor++;
			
			//UE_LOG(LogTemp, Warning, TEXT("WORLD TYPE:\t%d"), (int)(wp->WorldType));
			// If in game mode, or game mode in editor (WorldType->Game, WorldType->PIE), log to console.
			/*if (wp->WorldType == EWorldType::GamePreview) {
				UE_LOG(LogTemp, Warning, TEXT("WORLD TYPE:\tGAME"));
			}*/
			//Euler to x,y,z
			//x = cos(yaw)*cos(pitch)
			//y = sin(yaw) *cos(pitch)
			//z = sin(pitch)
			//UE_LOG(LogTemp, Warning, TEXT("MotionController FName is %s"), *str);
		}
		/*for (TActorIterator<AStaticMeshActor> ActorItr(GEngine->GetWorld()); ActorItr; ++ActorItr)
		{
			// Same as with the Object Iterator, access the subclass instance with the * or -> operators.
			AStaticMeshActor *Mesh = *ActorItr;
			FString str_actor = ActorItr->GetName();
			UE_LOG(LogTemp, Warning, TEXT("Actor name :%s"), *str_actor);
